...
proto.generateClient = function(modules, options, callback){
    ...
    this.configClient(options, function(err, clientOpts){
        me.serializeModules(modules, function(err, moduleSchemas ){
            ...
            var clientLoggerFile = require("path").resolve(__dirname , "client/angoose-client-logger.js");
            var clientLoggerFileContent = fs.readFileSync(clientLoggerFile , 'ascii');
            clientLoggerFileContent = clientLoggerFileContent.replace("/**CLIENT_LOGGER_LEVEL*/", "ClientLogger.levels."+options.logging);
            content +=clientLoggerFileContent;
            ...
            var jsonSchemasContent = fs.readFileSync(template , 'ascii');
            jsonSchemasContent = jsonSchemasContent.replace("/**SCHEMA_PLACEHOLDER*/",  jsonSchemas);
            /** angoose options */
            jsonSchemasContent = jsonSchemasContent.replace("/**CONFIG_PLACEHOLDER*/", JSON.stringify(clientOpts));
            content +=jsonSchemasContent;
