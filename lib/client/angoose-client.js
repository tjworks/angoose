...
(function() {
    ...
    AngooseClient.instanceInvoker = function(model, methodName, methodArgs) {
        var modelClass = AngooseClient.getClass(model.classname$)
    ...
    AngooseClient.invoker = function(modelClass, methodName, methodArgs, modelInstance) {
        ...
        data = encode$(data);
        console.log("====== Client RMI Call #", data.seqnumber,modelName, methodName, data);
    ...
    AngooseClient.getClass = AngooseClient.model = function(name) {
        if (!AngooseClient.initialized)
