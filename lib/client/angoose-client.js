...
(function BootstrapAngoose() {
    ...
    function compile(modelName, schema, dependencies) {
        ...
        // this is to be compatible with backend mongoose
    ...
    function config(path, val) {
        ...
        if (!path)
            return options;
        /**@todo: probably a deep copy */

        if ( typeof (path) === 'string') {
            if (val === undefined)
                return getter(options, path);
            setter(options, path, val);
        }
    }
