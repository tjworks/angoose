...
function BootstrapAngoose(){
    var logger = ClientLogger;
       ...
       logger.log("Bootstraping angoose-client");
    ...
    function invoker(modelClass, methodName, methodArgs, modelInstance) {
        ...
        var modelName =  modelClass.modelName;
         logger.log("invoker: ", modelName, methodName, methodArgs, modelClass);
        var callback = null;
        ...
        data = encode$(data);
         logger.log("****** BEGIN Client Call #", data.seqnumber,modelName, methodName, data);
        ...
        ret.done(function(retdata) {
             logger.log("****** END Client Call #", data.seqnumber, " result:", retdata);
            var val = undefined;
            ...
            if (retdata && retdata.instance && !isStatic) {
                ...
                modelInstance.mergeData(retdata.instance);
                 logger.log("Merged server side data");
            }
            if(retdata.success)
            ...
            //deferred.reject(ex)
             ...
             logger.log("****** END Client Call #", data.seqnumber, " error:", errdata);
    ...
    function compile(modelName, schema, dependencies) {
         logger.log("Client: compiling schema " , modelName)
    ...
    function init(dependencies) {
        if (AngooseClient.initialized) {
             logger.log("init has been called");
            return;
        }
        AngooseClient.dependencies = dependencies;
         logger.log("Initializing client angoose models")
