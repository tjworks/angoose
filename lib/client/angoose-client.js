...
(function BootstrapAngoose() {
    var angooseSchemas= /**SCHEMA_PLACEHOLDER*/;
    parseAllFunctionString(angooseSchemas);// parse function string to function
    ...
    function parseFunction(functionString){
        var fn=null;
        if(/^\s*function/.test(functionString)){
            eval("fn=" + functionString);
            return fn;
        }
        return fn;
    }
    function parseAllFunctionString(schema){
        for(var key in schema){
            if(typeof schema[key] == 'string'){
                var fn = parseFunction(schema[key])
                if(fn){
                    schema[key] = fn;
                }
            }else if (typeof schema[key] == 'object'){
                parseAllFunctionString(schema[key] )
            }
        }
    }
    ...
    function init(dependencies) {
        ...
        for (var mName in angooseSchemas) {
            compile(mName, angooseSchemas[mName], dependencies);
        }
    ...
    var configs = /**CONFIG_PLACEHOLDER*/ 
