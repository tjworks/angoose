...
proto.configClient = function(client){
    ...
    var clientOptionNames = ['url-prefix'];
    // client.options = traverse(angoose().config()).map(function(item){
         // //if(typeof(item) == 'function' || this.circular) this.remove();
         // if(this.key && allowed.indexOf(this.key) >= 0 ) {
             // client.options
         // };
    // });
    client.options.urlPrefix = angoose().config()['url-prefix'];
    client.options['url-prefix'] = angoose().config()['url-prefix'];
