...
var init = function  (app, conf) {
    ...
    app && app.post(configs.urlPrefix+"/rmi/:model/:method", function(req,res){
        ...
        try{
            ...
            var declaredArguments = schemaUtil.parseDeclaredArguments(target[method]);
            if(declaredArguments && declaredArguments.indexOf("$context")>=0){
                logger.debug("Injecting $context dependency");
                var ctx = new AngooseContext(req, invocation);
                args.push(ctx);
            } 
