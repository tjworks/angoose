...
var connectMongo = function(){
    try{
        logger.debug("Connecting to mongodb", configs.mongo_opts)
        mongoose.connect( configs.mongo_opts , function(err){
...
var modelErrorHandler = function(err, obj){
...
var rmiServer = function(req,res){
       ...
       //targetLoader.promise.done(function(target){
       function invokeMethod(target){
            //var target = new modelClass(invocation.instance);
            try{
...
var init = function  (app, conf) {
    ...
    mongoose.connection.once('open', function(err, result){
        logger.debug("mongoose connected", err, result)
    });
    app && app.post(configs.urlPrefix+"/rmi/:model/:method", rmiServer);
