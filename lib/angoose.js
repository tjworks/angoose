...
function lookupOrRegister(name, target){
    switch(arguments.length){
        ...
        case 2: // register a service
            if(typeof(target) === 'function' && target.schema  && target.modelName ){
                if(!beans[name]) target = angoose.Model.extend(target)                           
            }
            else{
...
function rmiAccept(req, res){
    ...
    execute(ctx, invocation, function(data){
        res.send(200, data);
    });
}
 ...
 */
function execute(ctx, invocation, callback){
        ...
        dmain.run(function(){
                ...
                function handleError(err, formated){
                    /**@todo: err may cause Circular issue when converting to JSON */                                
                     ...
                     callback( formated || formatError( {message: err.toString(), value:err, code:500 }, ctx.seqnum  ) )
                     sent  = true;
                }
    ...
    files.forEach(function(file) {
            ...
            try{
                ...
                if(modelClaz && modelClaz._angoosemeta){
                    ...
                    if(!beans[name]) registerClass(name, modelClaz)
                }
                    ...
                    logger.trace("Skip non-Angoose class file", filename)   
                }
            }
            catch(ex){
                 logger.error("Skipping file ", filename, ".js due to error: ", ex);
            }
    });   
};
// ** geneateClient() **
...
function registerHooks(){
    ...
    var extensions = options.hooks || options.extensions;
    if(extensions){
