...
function getContext(){
    ...
    if(!ctx){
        console.trace();
        throw "Context is not available. This may happen if the execution was not orginated by Angoose"  
    } 
    logger.debug("Get Context",ctx.seqnum);
    return ctx;
} 
...
function rmiAccept(req,res){
        ...
        excutionStorage.run(function(){
                ...
                var ctx = new angoose.Context({request:req, response:res, invocation:invocation, seqnum:seqnum });
                var sent = false;
                function handleError(err, formated){
                     res.send(200,  formated || formatError( {message: err.toString(), value:err, code:500 }, invocation.seqnumber ));
                     sent  = true;
                }
                /** function wrapper for handling try/catch. probar(spanish) == try  */ 
                function probar(fn, scope){
                    return function(){
                        try{
                            logger.debug("angoose step: ", (schemaUtil.getFunctionName(fn) || (fn._angoosemeta && fn._angoosemeta.funcName)  ));
                            fn.apply(scope, arguments);    
                        }
                        catch(ex){
                            logger.error("Uncaught error", ex);
                            handleError(ex, formatError(   {message: ex.toString(), value:ex, code:501 },  seqnum ));
                        }
                    }
                }
                var modelName = invocation.clazz;
                ...
                pipeline.authorize(invocation, ctx,  function(err, allowed){
                    if(err) return handleError(err);
                    logger.debug("authorized: ", allowed);
                    if(!allowed) return handleError("Access Denied");
                    
                    pipeline.resolveTarget(invocation, ctx, function(err, target){
                        if(err) return handleError(err);
                        
                        if(!target) return handleError("Unable to resolve target");
                        invocation.target = target;
                        
                        pipeline.resolveArguments(invocation, ctx, function(err, args){
                            if(err) return handleError(err);
                            invocation.args = args;
                            pipeline.invoke(invocation, ctx , function(ex, invocationResult){
                                if(ex) return handleError(ex);
                                pipeline.redact(invocation, ctx, invocationResult, function(err, redactionResult){
                                    if(err) return handleError(err);
                                    pipeline.pack(invocation, ctx, redactionResult, function(err, packedData){
                                        if(err) return handleError(err);
