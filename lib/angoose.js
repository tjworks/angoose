...
function registerExtension(name, module){
     ...
     return lookupOrRegister(name,module);      
}
...
function registerClass(nameOrOpts, claz){
    ...
    logger.info("Registered ", claz._angoosemeta.baseClass, className, (nameOrOpts.isExtension?'(extension)':''));
    return claz;
    // if(claz._angoosemeta && (claz._angoosemeta.baseClass == 'Service' || claz._angoosemeta.baseClass == 'Model') ){
//         
    // }
...
function harvestModules(options){
    ...
    var extensions = _.map(options['core-extensions'] , function(ext){
         return path.resolve(__dirname,  "../extensions/"+ ext);
    });
    extensions = extensions.concat(options.extensions || []);
    var files =  scanModelFiles( options.modelDir || options['module-dirs'] );
    extensions.concat(files).forEach(function(file) {
            try{
