...
// 
// Register a Remotable class with Angoose. You do not have to call this method if you created
// Model or Service class using the `$extend` function.  Mongoose models are also automatically registered
// upon initialization  
//
// Parameters:
...
function configureRoutes(app, options){
    ...
    app.post(options.urlPrefix+"/rmi/:model/:method", rmiServer);
...
function sendError (ex, seqnumber,res){
...
function rmiServer(req,res){
        /** setting execution context */
        var ctx = new angoose.Context({request:req, response:res});
        var excutionStorage = storageFactory.createNamespace("angoose");
        excutionStorage.run(function(){
            logger.debug("Setting context for user")
            excutionStorage.set("context", ctx);
            processRMI(req, res, ctx); 
        });
}
function processRMI(req, res, ctx){         
        var invocation = decode(req.body);
        var modelName = req.params['model'];
        /** method may be specified as part of the data object*/
        var method = req.params['method'] || invocation.method;  
        var seqnum = invocation.seqnumber;
        logger.debug("====== RMI Invocation #", seqnum, modelName, invocation );
        ...
        var targetLoader = Q.defer();
        
        if(invocation.static)
            invokeMethod(modelClass)
        else{
            
            /**@todo hacking depopulate for save method*/
            if(method === 'save'){
                /** depopulate the instance if necessary, otherwise Mongoose will complain*/
                Object.keys(modelClass.schema.paths).forEach(function(path){
                    var schemaData = modelClass.schema.paths[path];
                    
                    if(schemaData.instance == 'ObjectID' && schemaData.options.ref){
                        if(invocation.instance[path]  && invocation.instance[path]._id){
                            console.log("#### setting ", path, invocation.instance[path])
                            invocation.instance[path] = invocation.instance[path]._id;
                        }
                    }
                })    
            }
            ...
            if(_id){
                    modelClass.findById(_id, function(err, pristineInstance){
                       if(err || !pristineInstance){
                           ...
                           return targetLoader.reject(err);
                       }
                       /**@todo: Ref handling?*/
                       Object.keys(invocation.instance).forEach(function(key){
                            if(key.indexOf("_")==0 || typeof (invocation.instance[key]) == 'function') return;
                            /**logger.trace("seting key", key, "to", invocation.instance[key]) */ 
                             pristineInstance.set(key , invocation.instance[key], true);
                       });
                       logger.debug("Loaded model and updated value ", pristineInstance);
                       invokeMethod(pristineInstance)
                    });
            }
            else
                invokeMethod(new modelClass(invocation.instance))
       }
           
       function invokeMethod(target){
            try{
                useCallback = useCallback || injectDependencies(target[method], args, ctx, callbackHandler);
                logger.trace("invoking method ",  method.toUpperCase() , " with arguments", args)
...
function buildClientSchemas(options){
    var schemas = ''
